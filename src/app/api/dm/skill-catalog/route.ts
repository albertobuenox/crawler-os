...
const CUSTOM_ROLL = { roll_min: 0, roll_max: 0, page_ref: 0 };
...
async function readCatalogRequest(request: Request): Promise<CatalogFields> {
  ...
  if (contentType.includes("multipart/form-data")) {
    ...
    return {
      ...
      animal_only: form.get("animal_only") === "true",
...
export async function POST(request: Request) {
  try {
    ...
    if (!body.name?.trim()) return NextResponse.json({ error: "Ponle un nombre a la skill." }, { status: 400 });
    if (!body.description?.trim()) {
      return NextResponse.json({ error: "Todas las habilidades necesitan una descripción." }, { status: 400 });
    }
...
export async function PATCH(request: Request) {
  try {
    ...
    if (body.name !== undefined && !body.name.trim()) {
      return NextResponse.json({ error: "Ponle un nombre a la skill." }, { status: 400 });
    }
    ...
    if (body.animal_only !== undefined) patch.animal_only = body.animal_only;
