...
export function IANavRail({ unread = 0 }: { unread?: number }) {
  ...
  return (
    <nav className="hidden w-[72px] shrink-0 flex-col items-center gap-2 border-r border-[var(--stroke-glass)] bg-[rgba(5,6,13,0.5)] py-4 lg:flex">
      <p className="mb-2 font-display text-[8px] tracking-[0.2em] text-[var(--cyan-400)]">IA</p>
      {iaNav.map(({ href, icon: Icon, label, glow }) => {
        const active = pathname === href || (href !== "/ia" && pathname.startsWith(href));
        return (
          <Link
            key={href}
            href={href}
...
export function IATopBar({ sessionCode, sessionName }: { sessionCode?: string; sessionName?: string }) {
  return (
